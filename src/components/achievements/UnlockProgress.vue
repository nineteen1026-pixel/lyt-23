...
import { unlocks, getAllThresholds, type Decoration, type Apron } from '@/data/unlocks';
...
function getThresholdIcon(threshold: number) {
...
const thresholdItems = computed<ThresholdItem[]>(() => {
  ...
  unlocks.decorations.forEach((d) => {
    if (!map.has(d.threshold)) {
      map.set(d.threshold, { threshold: d.threshold, decorations: [], aprons: [] });
    }
  ...
  unlocks.aprons.forEach((a) => {
    if (!map.has(a.threshold)) {
      map.set(a.threshold, { threshold: a.threshold, decorations: [], aprons: [] });
    }
  ...
  <div class="space-y-5">
    ...
    <div class="card-soft p-5">
      <div class="flex items-center justify-between mb-4">
        ...
        <span class="text-sm text-brown-800/70">第 {{ store.totalDays }} 天</span>
      </div>

      <div class="relative pt-8 pb-4 px-2">
        <div
          class="absolute left-2 right-2 top-10 h-4 rounded-full"
        ...
        >
          <div
            ...
            :class="isThresholdReached(threshold)
          ...
          <span
            ...
            :class="isThresholdReached(threshold) ? 'text-apricot-600' : 'text-brown-800/40'"
          >
            {{ threshold }}天
          </span>
        </div>
